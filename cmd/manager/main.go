...
func main() {
	...
	ps, _ := platform.NewService(pt, mgr.GetScheme())
	ctrlLog := ctrl.Log.WithName("controllers")
	if err = (&gerritContr.ReconcileGerrit{
		Client:  mgr.GetClient(),
		Scheme:  mgr.GetScheme(),
		Service: gerrit.NewComponentService(ps, mgr.GetClient(), scheme),
		Log:     ctrlLog.WithName("gerrit"),
	}).SetupWithManager(mgr); err != nil {
		setupLog.Error(err, "unable to create controller", "controller", "gerrit")
		os.Exit(1)
	}

	if err = (&gerritreplicationconfig.ReconcileGerritReplicationConfig{
		Client:           mgr.GetClient(),
		Scheme:           mgr.GetScheme(),
		Platform:         ps,
		ComponentService: gerrit.NewComponentService(ps, mgr.GetClient(), mgr.GetScheme()),
		Log:              ctrlLog.WithName("gerrit-replication-config"),
	}).SetupWithManager(mgr); err != nil {
